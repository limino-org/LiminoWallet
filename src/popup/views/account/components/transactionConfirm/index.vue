<template>
  <div class="transactionConfirmComponent">
    <div class="text-center tit">{{t('transactionConfirmComponent.amount')}}</div>
      ...
      <div :class="`transactionConfirmComponent-con van-hairline--surround`">
        <div class="card-top">
          <div class="flex between gasfree">
            <div class="label">
              <van-skeleton :loading="loading" class="load" :row="1">
                {{t('transactionConfirmComponent.estimatedgassfree')}}
                <van-icon name="info" />
              </van-skeleton>
            </div>
          ...
          <div class="flex between maxfee">
            <div class="label">
              <van-skeleton :loading="loading" class="load" :row="1">
                {{t('transactionConfirmComponent.inSecnds')}}
              </van-skeleton>
            </div>
            <div class="value text-right">
              <van-skeleton
                ...
                :row="1"
              >
              {{t('transactionConfirmComponent.maxfee')}}
               {{decimal(ethTractionPrice(data.gas, data.gasPrice))}} {{currentNetwork.currencySymbol}}</van-skeleton>
            </div>
          </div>
        </div>
        ...
        <div class="card-bottom">
          <div class="label center-v">
            <span>
               {{t('transactionConfirmComponent.total')}}
            </span>
          </div>
          <div class="text-right info">
            ...
            <div class="maxacc">
              <van-skeleton
                ...
                :row="1"
              >
               {{t('transactionConfirmComponent.maxaccount')}} {{gasTotal(data.deposit, ethTractionPrice(data.gas, data.gasPrice))}} {{currentNetwork.currencySymbol}}</van-skeleton>
            </div>
          </div>
        </div>
      </div>
...
import { ethTractionPrice, getestimateGas, gasTotal, decimal } from '@/popup/utils/filters'
