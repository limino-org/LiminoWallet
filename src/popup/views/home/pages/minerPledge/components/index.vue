...
import AmountView from '@/popup/components/minerpledge/amount.vue'
...
export default {
  ...
  setup(props: any, context: SetupContext) {
    ...
    const { sendToPledge, currentRate } = useExchanges()
    const toCreate = async (name: string, amount: number, isServer: boolean) => {
      try {
        const dealMoney =
          Math.floor(amount) === Number(Number(accountInfo.value.amount).toFixed(2)) ? Math.floor(amount) - 1 : Math.floor(amount)
        await sendToPledge(name, dealMoney, isServer)
      } catch (err) {
        console.error(err)
      }
    }
    ...
    const submitCheck = (type: string, flag: Boolean) => {
      console.log('Get the returned type', type)
      if (type === 'check' && flag) {
        checked.value = true
      } else {
        checked.value = false
  ...
  .miners {
    ...
    .miners-container {
      ::-webkit-input-placeholder {
        /* WebKit browsers，webkit kernel browser */
        color: #232323;
        font-size: 12px;
      }
      :-moz-placeholder {
